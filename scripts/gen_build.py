...
with pl.project("pilotlight_python"):
    ...
    with pl.target("imgui", pl.TargetType.STATIC_LIBRARY, False, False):
        ...
        with pl.configuration("deploy"):
            ...
            with pl.platform("Windows"):
                with pl.compiler("msvc"):
                    pl.add_linker_flags("-incremental:no", "-nologo")
                    pl.add_compiler_flags("-nologo", "-std:c++14", "-WX", "-O2", "-MD", "-Zi", "-permissive")
    ...
    with pl.target("pilotlight_python", pl.TargetType.EXECUTABLE, False):
        ...
        with pl.configuration("debug"):
            ...
            with pl.platform("Linux"):
                ...
                with pl.compiler("gcc"):
                    pl.add_definitions("PL_VULKAN_BACKEND", "LINUX", "UNIX")
        ...
        with pl.configuration("debug"):
            ...
            with pl.platform("Windows"):
                with pl.compiler("msvc"):
                    ...
                    pl.add_static_link_libraries("user32", "Shell32", "Ole32", "gdi32", "ucrtd", "pl_platform_ext")
                    pl.add_link_directories("../dependencies/cpython/PCbuild/amd64/", '%VULKAN_SDK%\\Lib')
                    pl.add_linker_flags("-noimplib", "-noexp", "-incremental:no", "-nodefaultlib:LIBCMT", "-nodefaultlib:MSVCRT")
                    pl.add_compiler_flags("-Zc:preprocessor", "-nologo", "-std:c11", "-W4", "-WX", "-wd4201",
                                            "-wd4100", "-wd4996", "-wd4505", "-wd4189", "-wd5105", "-wd4115",
                                            "-permissive-", "-Od", "-MDd", "-Zi")
        ...
        with pl.configuration("debug"):
            ...
            with pl.platform("Linux"):
                with pl.compiler("gcc"):
                    ...
                    pl.add_compiler_flags("-fPIC")
                    pl.add_compiler_flags("--debug -g", "-std=c++14")
                    pl.add_linker_flags("-ldl -lm", "-lstdc++")
                    pl.add_dynamic_link_libraries("shaderc_shared", "spirv-cross-c-shared", "vulkan")
                    pl.add_static_link_libraries("glfwd", "pl_platform_ext")
                    pl.add_link_directories("../dependencies/cpython/build/debug/", '$VULKAN_SDK/lib')
    ...
    with pl.target("pilotlight", pl.TargetType.DYNAMIC_LIBRARY, False):
        ...
        pl.set_output_directory("../pilotlight")
        pl.add_static_link_libraries("pilotlight_main")
